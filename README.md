...
- API Key 优先存储在系统凭据库（Keychain/Credential Vault），不可用时回退到本地数据库加密存储。
...
- **安全配置升级**：AI API Key 优先写入系统凭据库，不可用时回退本地数据库加密。
...
- API Key 优先存储在系统凭据库（Keychain/Credential Vault）。
...
- API Key 优先保存在系统凭据库；若系统凭据库不可用，回退本地数据库加密存储。
